...
public class CanonShoot3 : MonoBehaviour {
    ...
    void Start()
    {
        ballTarget = GameObject.FindGameObjectWithTag("hit3");
    }
    ...
    void Update()
    {
        ...
        if (canonActivated && canonCanShoot)
        {
            ...
            if (MoneyCount.Money > 200)
            {
                ...
                PhotonState.typeBall = 3;
                PhotonState.posBall = ballTarget.transform.position;
                //new photon
                //     DemoGame.getInstance().SendSpawnBall(true, 3, ballTarget.transform.position);
