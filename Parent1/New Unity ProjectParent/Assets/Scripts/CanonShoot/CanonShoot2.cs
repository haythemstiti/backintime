...
public class CanonShoot2 :MonoBehaviour
{
    ...
    void Start()
    {
        ballTarget = GameObject.FindGameObjectWithTag("hit2");
    }
    ...
    { 
        ...
        if (canonActivated && canonCanShoot)
        {
            ...
            if (MoneyCount.Money > 200)
            {
                ...
                PhotonState.typeBall = 2;
                PhotonState.posBall = ballTarget.transform.position;
                //new photon
                //    DemoGame.getInstance().SendSpawnBall(true, 2, ballTarget.transform.position);
